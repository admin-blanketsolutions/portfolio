# Audit Management Platform: Phase 1 secure core + Phase 3 TB ingestion (slice 1)

A multi-tenant SaaS core for audit firms (ISA / ISQM, IFRS, ISO/IEC 27001, Jordan & MENA residency). The platform covers engagement workflow, AI-assisted trial-balance mapping, digital workpapers with 3-tier sign-off, sampling, and FS generation from TB + AJEs. This directory holds the **architecture review, the roadmap, and the executed Phase 1 foundation**: the PostgreSQL schema with the security model enforced in the database, and the TypeScript/NestJS tenant-context runtime. It also holds the first Phase 3 slice: trial-balance import and AI-assisted account mapping.
...
│   │   └── modules/      engagements, tb-ingestion (upload, worker, cascade, LLM stage, review API), health
│   └── test/             unit (106) · integration (8) · HTTP e2e (8 + 11 TB ingestion)
└── docs/
```
